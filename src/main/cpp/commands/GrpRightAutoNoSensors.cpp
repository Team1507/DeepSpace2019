...
#define DRIVE_STR8_DISTANCE  169 //The first Drive Foward //was 235 / 194
...
#define LAST_TURN_ANGLE      -205 //will change with the booking angle was -185
#define BOOKING_SPEED        0.5 //was 0.7
#define BOOKING_ANGLE        -185 //may be totally wrong
#define BOOKING_DISTANCE     96 //its whatever //was 48

//******Auto Group******
GrpRightAutoNoSensors::GrpRightAutoNoSensors() 
{
    AddSequential(new CmdPrintAutoText("GrpRightAutoNoSensors Begin"));
    ...
    AddSequential(new CmdDriveFwdGyro(DRIVE_SPEED, 2, DRIVE_STR8_DISTANCE, true, 0.0));
    //Slow down
    //AddSequential(new CmdDriveManual(-0.2, -0.2, 0.5));
    //Turn
    AddSequential(new CmdDriveTurn2Heading(-0.30, .6, -90.0, 5.0 )); 
    //Turn left until line is found
    //AddSequential(new CmdTurn2Line(true, TURN_2_LINE_SPEED));
    //Drive fowards
    //AddSequential(new CmdDriveFwdGyro(.3, -90, 6, true, 2.0)); //timeout was 4
    //AddSequential(new CmdDriveLineDriftLeft(LINE_FOLLOW_SPEED, -90, LINE_FOLLOW_DISTANCE, true, 4.0)); //timeout was 0.0
  
    AddSequential(new CmdDriveManual(0.0, 0.0, 2.0));
    ...
    //start booking
    AddSequential(new CmdDriveFwdGyro(BOOKING_SPEED, LAST_TURN_ANGLE, BOOKING_DISTANCE, true, 0.0));//will want to fiddle with timeout
}
