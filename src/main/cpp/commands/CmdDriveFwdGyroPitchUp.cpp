...
const double PITCH_LIMIT = -1.0;

CmdDriveFwdGyroPitchUp::CmdDriveFwdGyroPitchUp(double power, double heading, double distance, bool stop, double timeout)
{
  m_power    = power;
	m_heading  = heading;
  m_distance = distance;
...
void CmdDriveFwdGyroPitchUp::Initialize() 
{
...
void CmdDriveFwdGyroPitchUp::Execute() 
{
...
bool CmdDriveFwdGyroPitchUp::IsFinished() 
{
	...
	{
		std::cout<<"CmdDriveFwdGyroPitchUp: Timeout"<<std::endl;
		return true;
	}
...
void CmdDriveFwdGyroPitchUp::End() 
{
  if(m_stop)
	{
		std::cout<<"GyroAngle ";
...
void CmdDriveFwdGyroPitchUp::Interrupted() 
{
  End();
}
